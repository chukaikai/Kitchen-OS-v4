(function(){
  ...
  const UPDATED_KEY="kitchen-os:item-settings-updated-at:v1";
  ...
  function localUpdatedAt(){ return localStorage.getItem(UPDATED_KEY)||""; }
  function write(data){
    const updatedAt=new Date().toISOString();
    writeLocal(data,updatedAt);
    const ref=cloudDoc();
    // V61: 先存本機，再等待雲端完成；避免按「儲存設定」後立刻 reload，
    // 被 Firestore 舊快照把剛取消勾選的品項重新覆蓋回來。
    if(ref) return ref.set({settingsJson:JSON.stringify(data),updatedAt},{merge:true})
      .catch(e=>{console.error("品項設定雲端同步失敗",e); throw e;});
  ...
  function configureCloud(db){
    ...
    cloudUnsub=ref.onSnapshot(snap=>{
      ...
      if(remote&&typeof remote==="object"){
        ...
        const remoteUpdated=String(d?.updatedAt||"");
        const localUpdated=localUpdatedAt();
        if(before!==after){
          if(localUpdated && (!remoteUpdated || localUpdated>remoteUpdated)){
            // 本機較新：保留剛剛的勾選狀態並補傳，不讓舊雲端資料蓋掉。
            ref.set({settingsJson:JSON.stringify(local),updatedAt:localUpdated},{merge:true}).catch(console.error);
          }else{
            writeLocal(remote,remoteUpdated||new Date().toISOString());
            if(!first)setTimeout(()=>location.reload(),50);
          }
        }
      }else if(first){
        const local=read();
        if(Object.keys(local).length){
          const ts=localUpdatedAt()||new Date().toISOString();
          writeLocal(local,ts);
          ref.set({settingsJson:JSON.stringify(local),updatedAt:ts},{merge:true}).catch(console.error);
        }
      }
  ...
  function set(type,station,value){const all=read();all[key(type,station)]=normalizeCfg(value);return write(all)}
  ...
  function openManager(type,station,base,onDone){
    ...
    shade.querySelector('[data-close]').onclick=()=>shade.remove();shade.querySelector('[data-reset]').onclick=()=>{ids=[...all.keys()];hidden.clear();draw()};shade.querySelector('[data-save]').onclick=async()=>{const btn=shade.querySelector('[data-save]');cfg.order=ids;cfg.hidden=[...hidden];btn.disabled=true;btn.textContent='儲存中…';try{await set(type,station,cfg);shade.remove();onDone&&onDone()}catch(e){btn.disabled=false;btn.textContent='儲存設定';alert('本機已儲存，但雲端同步失敗。請確認網路後再試一次。')}};document.body.append(shade);draw();
  }
  window.KitchenItemSettings={KEY,UPDATED_KEY,TYPES,STATIONS,read,write,get,set,idFor,apply,openManager,configureCloud,cloudReady};
